...
export default class LinkUI extends Plugin {
	...
	_showUI() {
		...
		window.l = linkCommand;
		window.u = unlinkCommand;
		...
		if ( selected ) {
			...
			console.warn( '4ui.,', selected );
		}
		// If theres a link under the selection...
		else {
			console.warn( '3no link yet?' );
		}

		const update = newValue => {
			console.warn( 123, { newValue } );
			// editor.editing.view.focus();
			//  setTimeout( () => {
			if ( currValue.href ) {
				// linkCommand.execute( newValue.href );
        unlinkCommand.execute( );
				// editor.execute( 'bold', true );
			} else {
